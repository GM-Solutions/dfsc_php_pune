...
//        print_r($product_info); 
        ...
        foreach ($op_raw as $key => $value) {
            ...
            $op['service_status'][$i]['veh_reg_no']=!empty($value['veh_reg_no']) ? $value['veh_reg_no'] : "";
            $op['service_status'][$i]['customer_id']=!empty($value['customer_id']) ? $value['customer_id'] :"";
    ...
    public function checkCoupon_post() {
        ...
        if(isset($product_info)){
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS,
                        "phoneNumber=".$mobile_no."&text=A ".$customer_id." ".$km." ".$service_type);
    ...
    public function customer_registration_post() {
        ...
        $group = $this->post('group');/*new value*/
        $user_id = $this->post('user_id');/*new value*/
        $customer_id = $this->post('customer-id');/*new value*/
        ...
        /* register customer API */
            ...
            $curl_url = $this->config->item('curl_api_custom');
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,$curl_url[$country]);
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS,
                        "customer-id=".$customer_id."&customer-name=".$owner_name."&purchase-date=".$purchase_date."&customer-vin=".$veh_reg_no."&customer-phone=".$owner_mobile_no."&group=".$group."&user_id=".$user_id);
    ...
    public function close_coupon_post() {
        ...
        $curl_url = $this->config->item('curl_api');
            ...
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS,
                        "phoneNumber=".$mobile_no."&text=C ".$customer_id." ".$ucn);
